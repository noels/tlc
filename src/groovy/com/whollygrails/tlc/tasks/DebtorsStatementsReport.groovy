    ...
    def execute() {
        ...
        if (params.customer) {
            ...
            if (bookService.hasCustomerAccess(specificCustomer, company, user)) {
                ...
                completionMessage = message(code: 'customer.not.found', args: [params.customer], default: "Customer not found with id ${params.customer}")
                return false
            }
        } else {
        ...
        reportParams.txtCustomer = message(code: 'customer.statement.customer', default: 'Specific Customer')
                ...
                if (customer && !Statement.countByCustomerAndStatementDateGreaterThanEquals(customer, statementDate)) {
                    ...
                    if (transactions) {
                        ...
                        for (tran in transactions) {
                            ...
                            type = doc.type.code
                            code = doc.code
                            val = new StatementLine(type: type, code: code, documentDate: doc.documentDate, dueDate: doc.dueDate, reference: doc.reference, originalValue: tran.accountValue,
                                    openingUnallocated: tran.reconciled ? tran.accountUnallocated : tran.accountValue, closingUnallocated: tran.accountUnallocated, sequencer: tran.id,
                                    source: tran, currentStatement: setStatus(postingCutoff, documentCutoff, cutoffDate, tran.reconciled, tran.dateCreated, doc.documentDate))
