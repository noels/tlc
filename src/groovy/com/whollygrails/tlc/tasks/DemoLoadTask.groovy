...
import com.whollygrails.tlc.corp.TaxCode
import doc.Line
...
class DemoLoadTask extends TaskExecutable {
    ...
    def execute() {
        ...
        if (!corp) {
            completionMessage = message(code: 'company.not.found', args: [params.companyId], default: "Company not found with id ${params.companyId}")
            return false
        }

        def usr = SystemUser.get(params.userId)
        if (!usr) {
            completionMessage = message(code: 'systemUser.not.found', args: [params.userId], default: "User not found with id ${params.userId}")
            return false
        }
        ...
        if (curr?.company?.id != corp.id) {
            completionMessage = message(code: 'exchangeCurrency.not.found', args: [params.currencyId], default: "Currency not found with id ${params.currencyId}")
            return false
        }

        def taxCode = TaxCode.get(params.taxCodeId)
        if (taxCode?.company?.id != corp.id) {
            completionMessage = message(code: 'taxCode.not.found', args: [params.taxCodeId], default: "Tax Code not found with id ${params.taxCodeId}")
            return false
        }
        ...
        Account.withTransaction {status ->
            ieSection = new ChartSection(company: corp, parentObject: null, path: 'ie', code: 'ie', name: 'Income and Expenditure', sequencer: 100, type: 'ie', status: 'cr').saveThis()
            salesSection = new ChartSection(company: corp, parentObject: ieSection, path: 'ie.sales', code: 'sales', name: 'Sales', sequencer: 100, type: 'ie',
                    segment1: accountElement, segment2: lobElement, segment3: officeElement, segment4: projectElement,
                    default3: 'CHO', default4: '000', status: 'cr').saveThis()
            new ChartSectionRange(section: salesSection, rangeFrom: '100000-*-*-*', rangeTo: '199999-*-*-*').saveThis()
            cosSection = new ChartSection(company: corp, parentObject: ieSection, path: 'ie.cos', code: 'cos', name: 'Cost of Sales', sequencer: 200, type: 'ie',
                    segment1: accountElement, segment2: lobElement, segment3: officeElement, segment4: projectElement,
                    default3: 'CHO', default4: '000', status: 'dr').saveThis()
            new ChartSectionRange(section: cosSection, rangeFrom: '200000-*-*-*', rangeTo: '299999-*-*-*').saveThis()
            expensesSection = new ChartSection(company: corp, parentObject: ieSection, path: 'ie.expenses', code: 'expenses', name: 'Expenses', sequencer: 300, type: 'ie',
                    segment1: accountElement, segment2: officeElement,
                    default2: 'CHO', status: 'dr').saveThis()
            new ChartSectionRange(section: expensesSection, rangeFrom: '300000-*', rangeTo: '399999-*').saveThis()
            otherSection = new ChartSection(company: corp, parentObject: ieSection, path: 'ie.other', code: 'other', name: 'Non-Trading Items', sequencer: 400, type: 'ie',
                    segment1: accountElement, status: 'dr').saveThis()
            new ChartSectionRange(section: otherSection, rangeFrom: '400000', rangeTo: '499999').saveThis()

            bsSection = new ChartSection(company: corp, parentObject: null, path: 'bs', code: 'bs', name: 'Balance Sheet', sequencer: 500, type: 'bs', status: 'dr').saveThis()
            fixedSection = new ChartSection(company: corp, parentObject: bsSection, path: 'bs.fixed', code: 'fixed', name: 'Fixed Assets', sequencer: 100, type: 'bs',
                    segment1: accountElement, status: 'dr').saveThis()
            new ChartSectionRange(section: fixedSection, rangeFrom: '500000', rangeTo: '599999').saveThis()
            netSection = new ChartSection(company: corp, parentObject: bsSection, path: 'bs.net', code: 'net', name: 'Net Current Assets', sequencer: 200, type: 'bs', status: 'dr').saveThis()
            caSection = new ChartSection(company: corp, parentObject: netSection, path: 'bs.net.ca', code: 'ca', name: 'Current Assets', sequencer: 100, type: 'bs',
                    segment1: accountElement, status: 'dr').saveThis()
            new ChartSectionRange(section: caSection, rangeFrom: '600000', rangeTo: '699999').saveThis()
            clSection = new ChartSection(company: corp, parentObject: netSection, path: 'bs.net.cl', code: 'cl', name: 'Current Liabilities', sequencer: 200, type: 'bs',
                    segment1: accountElement, status: 'cr').saveThis()
            new ChartSectionRange(section: clSection, rangeFrom: '700000', rangeTo: '799999').saveThis()
            nonSection = new ChartSection(company: corp, parentObject: bsSection, path: 'bs.non', code: 'non', name: 'Non-Current Assets', sequencer: 300, type: 'bs',
                    segment1: accountElement, status: 'dr').saveThis()
            new ChartSectionRange(section: nonSection, rangeFrom: '800000', rangeTo: '899999').saveThis()
            equitySection = new ChartSection(company: corp, parentObject: bsSection, path: 'bs.equity', code: 'equity', name: "Shareholder's Equity", sequencer: 400, type: 'bs',
                    segment1: accountElement, status: 'cr').saveThis()
        ...
        if (docType) {
            ...
            jnl.addToLines(new Line(account: buildingsCostAccount, documentValue: 6239107.0, generalValue: 6239107.0, companyValue: 6239107.0, description: description))
            jnl.addToLines(new Line(account: fnfCostAccount, documentValue: 704219.0, generalValue: 704219.0, companyValue: 704219.0, description: description))
            jnl.addToLines(new Line(account: carsCostAccount, documentValue: 1138002.0, generalValue: 1138002.0, companyValue: 1138002.0, description: description))
            jnl.addToLines(new Line(account: inventoryAccount, documentValue: 520798.0, generalValue: 520798.0, companyValue: 520798.0, description: description))
            jnl.addToLines(new Line(account: cashAccount, documentValue: 5000.0, generalValue: 5000.0, companyValue: 5000.0, description: description))
            jnl.addToLines(new Line(account: bankAccount, documentValue: 1392874.0, generalValue: 1392874.0, companyValue: 1392874.0, description: description))
            jnl.addToLines(new Line(account: capitalAccount, documentValue: -10000000.0, generalValue: -10000000.0, companyValue: -10000000.0, description: description))
