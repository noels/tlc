...
public class RevaluationTask extends TaskExecutable {
    def execute() {
        ...
        if (!period || period.securityCode != company.securityCode || !activePeriods.find {it.code == period.code}) {
            completionMessage = message(code: 'period.not.found', args: [params.periodId], default: "Period not found with id ${params.periodId}")
            return false
        }
        ...
        if (!fxControl || fxControl.securityCode != company.securityCode || !['fxDiff', 'fxRevalue'].contains(fxControl.type?.code)) {
            completionMessage = message(code: 'account.not.found', args: [params.targetId], default: "Account not found with id ${params.targetId}")
            return false
        }
        ...
        if (count) {
            ...
            for (tot in totals) {
                ...
                line = new Line(account: control, documentValue: tot.value, generalValue: tot.value, companyValue: tot.value, adjustment: adjustment)
                reversalLine = new Line(account: control, documentValue: -tot.value, generalValue: -tot.value, companyValue: -tot.value, adjustment: adjustment)
            ...
            if (description.length() > 50) description = description.substring(0, 50)
            line = new Line(account: fxControl, description: description, documentValue: -grandTotal, generalValue: -grandTotal, companyValue: -grandTotal, adjustment: adjustment)
            document.addToLines(line)
            description = message(code: 'document.revaluation.diff.reversal', default: 'FX Reversal')
            if (description.length() > 50) description = description.substring(0, 50)
            reversalLine = new Line(account: fxControl, description: description, documentValue: grandTotal, generalValue: grandTotal, companyValue: grandTotal, adjustment: adjustment)
            reversalDocument.addToLines(reversalLine)
            ...
            if (valid) {
                ...
                reportParams.put('txtAccount', message(code: 'document.line.accountCode', default: 'Account'))
                reportParams.put('txtName', message(code: 'account.name', default: 'Name'))
