...
class CustomerListReport extends TaskExecutable {
    ...
    def execute() {
        ...
        def title = message(code: 'customer.list', default: 'Customer List')
        reportParams.put('reportTitle', title)
        yield()
        def pdfFile = createReportPDF('CustomerList', reportParams)
        yield()
