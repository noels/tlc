...
import com.whollygrails.tlc.corp.TaxAuthority
import com.whollygrails.tlc.corp.TaxCode
import com.whollygrails.tlc.corp.TaxStatement
import com.whollygrails.tlc.corp.TaxStatementLine
...
class TaxStatementTask extends TaskExecutable {
    ...
    def execute() {
        ...
        if (!taxAuthority || taxAuthority.securityCode != company.securityCode) {
            completionMessage = message(code: 'taxAuthority.not.found', args: [params.authority], default: "Tax Authority not found with id ${params.authority}")
            return false
        }
...
// --------------------------------------------- Support Methods ---------------------------------------------
    ...
    private summarize(statement, current, input) {
        ...
        GeneralTransaction.executeQuery(sql, ['T' + statement.authority.id.toString(), statement.statementDate, statement.priorDate]).each {
