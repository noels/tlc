...
// --------------------------------------------- Static Methods ----------------------------------------------
    ...
    static syncActionActivities() {
        Logger log = Logger.getLogger(SecurityService)
        def activities

        // Work through each controller in the system
        ApplicationHolder.getApplication().getArtefacts(ControllerArtefactHandler.TYPE).each {ctrl ->
            def controller = GrailsClassUtils.getPropertyName(ctrl.getName())
            ...
            if (activities) {
                ...
                activities.each {
                    log.error("Controller ${controller} refers to a non-existent action ${it.key} with an activity of ${it.value}")
                }
    ...
    // Ensure the company administrator role has all non-system permissions
    static syncCompanyAdminRole() {
        def map = [:]

        // Get all non-system activity codes in to a map for fast lookup
        SystemActivity.findAllBySystemOnly(false).each {
            map.put(it.code, null)
        }

        // Get all activity codes for the company administrator role
        def list = SystemActivity.executeQuery('select a.code from SystemRole as r join r.activities as a where r.code = ?', ['companyAdmin'])

        // Knock out from the map all activities that the compamy administrator role already posesses
        list.each {
            map.remove(it)
        }

        // Whatever is left in the map are activities that need adding to the company administrator role
        if (map) {
            def role = SystemRole.findByCode('companyAdmin')
            if (role) {
                map.each {
                    role.addToActivities(SystemActivity.findByCode(it.key))
                }
    ...
    static syncTracingDomains() {

        // Work through each domain class in the system
        ApplicationHolder.getApplication().getArtefacts(DomainClassArtefactHandler.TYPE).each {dom ->
            ...
            if (domain.class.simpleName != 'SystemTrace' && domain.class.simpleName != 'SystemWorkarea') {
                ...
                if (systemTracingInstance) {
                    ...
                    if (traceable != null) {
                        ...
                        systemTracingInstance.systemOnly = domain.class.name.startsWith('com.whollygrails.tlc.sys.')
...
// --------------------------------------------- Support Methods ---------------------------------------------
    ...
    private loadUserAccessGroups(company, user) {
        ...
        def groups = []
        def size = 0
        usr.accessGroups.each {
            groups << it.code
            size += it.code.length()
        }

        return [size ?: 1, groups]
    }
