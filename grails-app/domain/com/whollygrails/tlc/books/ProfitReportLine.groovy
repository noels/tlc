...
class ProfitReportLine {
    ...
    static belongsTo = [format: ProfitReportFormat, section: ChartSection]
    ...
    static constraints = {
        ...
        section(nullable: true, validator: {val, obj ->
            if (val) {
                if (val.securityCode != obj.format.securityCode || val.type != 'ie' || val.accountSegment <= 0) return ['invalid', obj.lineNumber, val.code]
            }
