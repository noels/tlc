...
class GeneralController {
    ...
    // Security settings
    def activities = [default: 'gltemplate', template: 'gljournal', journal: 'gljournal', lines: 'gljournal', journaling: 'gljournal', enquire: 'enquire']

    // List of actions with specific request types
    static allowedMethods = [delete: 'POST', save: 'POST', update: 'POST', templateLines: 'POST', lines: 'POST', journaling: 'POST']
    ...
    def list = {
        ...
        def listing = TemplateDocument.findAll("from TemplateDocument as x where x.type.company = ? and x.type.type.code = 'GLJ' order by x.type.code, x.description",
                [utilService.currentCompany()], [max: max, offset: offset])
        def total = TemplateDocument.executeQuery("select count(*) from TemplateDocument as x where x.type.company = ? and x.type.type.code = 'GLJ'", [utilService.currentCompany()])[0]
    ...
    def show = {
        ...
        if (!templateDocumentInstance || templateDocumentInstance.type.type.code != 'GLJ') {
    ...
    def journaling = {
        ...
        if (valid) {           // Document comes back from posting in its debit/credit form
            ...
            render(view: 'journal', model: getModel(company, documentInstance))
        }
    }

    def enquire = {
        def model = bookService.loadDocumentModel(params, ['GLJ'])
